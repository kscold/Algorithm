...
public class q1
{
    public static int solution(int S, int N, int[][] skills)
    {
        // your code is here
        // -----------------------------------
        // -----------------------------------
    ...
    public static void main(String[] args)
    {
        ...
        try {
            ...
            for(int i=0; i<K; i++) {
                datain.next();
                int S = datain.nextInt();
                int N = datain.nextInt();
                datain.nextLine();
                int skills[][] = new int[N][S];
                for(int j=0; j<N; j++) {
                    String line = datain.nextLine();
                    Scanner skill_in = new Scanner(line);
                    int m = 0;
                    while(skill_in.hasNextInt()) {
                        int skill = skill_in.nextInt();
                        skills[j][m] = skill;
                        m = m + 1;
                    }
                }
                int result = solution(S, N, skills);
