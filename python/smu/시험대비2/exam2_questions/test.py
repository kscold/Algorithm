...
def q1(coinmap):
    ...
    #
    # def dfs(arr, x, y, idx):  # 코인맵과 좌표, 초기 카운트를 대입
    #     nonlocal result
    ...
    #             continue
    #         if (arr[nx][ny] == 'O'):  # 코인을 만나면 줍기
    #             # result += 1
    #             result = 1
    #             # cnt = 1
    #             # cnt += dfs(coinmap, nx, ny, idx + 1)
    #         else:
    #             result += dfs(coinmap, nx, ny, idx + 1)
    #             # cnt += 1
    #             # print("sdf")
    #
    #     return cnt
    #
    # # return dfs(coinmap, 0, 0, 1)
    #
    # for x in range(R):  # 가로 크기
    #     for y in range(C):
    #         if coinmap[x][y] == 'O':  # 지뢰가 없으면서 열린 칸
    #             return dfs(coinmap, 0, 0, 1)
    #
    # arr = [['E', 'C', 'C', 'C', 'C'],
    #        ['C', '#', 'C', '#', 'E'],
